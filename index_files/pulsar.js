...
getPlsUBTBrowser=function(){var b=navigator.userAgent?navigator.userAgent.toLowerCase().replace(/-/g,''):'';var bs=['chrome','firefox','safari','msie','opera'];for(var i=0;i<bs.length;i+=1){if(b.indexOf(bs[i])!==-1){return bs[i];}}};_plsubtInp.browser=getPlsUBTBrowser();_plsubtInp.plsUBT=1;_plsubtInp.resolut=screen.width+'x'+screen.height;_plsubtInp.scrColDep=screen.colorDepth;_plsubtInp.tiZone=(new Date().getTimezoneOffset()/60).toString();isPlsUBTInSampling=function(){var isInSamp=false;if(_plsubtInp.samplingRate!=0){try{isInSamp=(CGuidHash.isInSampling(_plsubtInp.samplingRate))?true:false;}catch(err){isInSampg=true;}}
...
function PlsUBTURLTemplate(inp,hostname,protocol){var baseURLTemplate='';if(inp.serverUrl!=null){baseURLTemplate=inp.serverUrl;}else{var serverHost;if(inp.env=="qa"||hostname.indexOf(".qa.")>-1){serverHost='www.pulsar.stratus.qa.ebay.com';}else{serverHost="pulsar.ebay.com";}
baseURLTemplate=protocol+"//"+serverHost;}
...
function firePulsarProxyURL(unload){_plsUBTPPURL=_plsubtInp.URLTemplate+"pld=";var paylaod="[";for(var i=0;i<_plsUBTpld.length;i++){_plsUBTpld[i].difTS=new Date().getTime()-_plsUBTpld[i].difTS;paylaod=paylaod+JSON.stringify(_plsUBTpld[i]);if(i!=_plsUBTpld.length-1){paylaod=paylaod+", ";}};paylaod=paylaod+"]";_plsUBTPPURL=_plsUBTPPURL+encodeURIComponent(paylaod);if(navigator.sendBeacon){sendBeacon(_plsUBTPPURL);}else{plsUBTAjaxCall(unload);}};firePulsarProxyURLAsImg=function(){for(var i=0;i<_plsUBTpld.length;i++){_plsUBTpld[i].difTS=new Date().getTime()-_plsUBTpld[i].difTS;var ppImpURL=_plsubtInp.URLTemplate+"pld="+encodeURIComponent("["+JSON.stringify(_plsUBTpld[i])+"]");var img=new Image();img.src=ppImpURL;}}
function postPlsUBTCALL(unload){if(_plsubtInp.isInSampling&&plsUBTAllReq){_plsUBTtaq.pop(_plsUBTtaq,window._plsUBTTQ);if(_plsUBTpld.length!=0){firePulsarProxyURL(unload);_plsUBTpld.length=0;}}};function sendWait(inp){if(inp==undefined||inp==null||inp.sendWait==undefined||inp.sendWait==null||isNaN(inp.sendWait)){return 60000;}else if(inp.sendWait<10000){return 10000;}else{return inp.sendWait;}}
if(_plsubtInp.browser=='chrome'||_plsubtInp.browser=='safari'){window.addEventListener("beforeunload",function doPost(){postPlsUBTCALL(true);});}else{window.addEventListener("unload",function doPost(){postPlsUBTCALL(true);});}
setInterval(function(){postPlsUBTCALL(false)},sendWait(_plsubtInp));function plsUBTAjaxCall(unload){var ajax=false;if(window.XDomainRequest){ajax=new window.XDomainRequest();}
if(ajax==false){if(window.XMLHttpRequest){ajax=new window.XMLHttpRequest();}else{ajax=new ActiveXObject("Microsoft.XMLHTTP");}}
var async;if(unload){async=false;}else{async=true;}
ajax.open("POST",_plsUBTPPURL+"&cache="+Math.random(),async);if(_plsubtInp.browser=='chrome'||_plsubtInp.browser=='safari'){ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");}
ajax.send("");}
// b=18006650 -->